...
def on_help_callback(client, callback_query):
    ...
    callback_query.message.edit_text(help_text)
...
def on_about_callback(client, callback_query):
    ...
    callback_query.message.edit_text(about_text)
...
def handle_remove_audio(client, message):
    ...
    if success_remove_audio:
        client.send_video(chat_id=message.chat.id, video=output_file_no_audio, progress=progress, progress_args=(progress_message, start_time))
    else:
...
def process_request():
    ...
    if action == 'remove_audio':
        ...
        success = executor.submit(trim_video, video_file, start_time, end_time, output_file).result()
    else:
